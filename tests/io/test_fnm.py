...
def test_serialize_parse_should_return_same_result():
    ...
    model.node_data = Path("../somewhere/else")
    model.bui_file = Path("bui_file.bui")
    model.green_house_general = Path("../../greenhouse.general")
    model.restart_file_input = Path("../res.res")
    model.meteo_input_file_rainfall = Path("some/path.ini")

    serialized_model = serialize(model)
    deserialized_model = parse(serialized_model.splitlines())
...
def test_parse_serialize_should_return_same_result():
    ...
    *
    ...
    '../res.res'                  *  41. Restart file input                                                   I
    ...
    'some/path.ini'               * 125. Optional meteo NetCdf timeseries inputfile rainfall

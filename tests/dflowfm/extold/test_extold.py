...
class TestOldExtForcingHelpers:
    @pytest.mark.parametrize(
        "filetype, expected_result",
        [
            pytest.param("poly", "named_node", id="polyfile-object-name"),
            pytest.param("tim", "triple_data_for_timeseries", id="timmodel-stem"),
            pytest.param("disk", "mydisk", id="diskonlyfilemodel-stem"),
            pytest.param("disk-none", "lateraldischarge", id="fallback-to-quantity"),
        ],
    )
    def test_get_location_id(
        ...
        if filetype == "poly":
            ...
            forcing = ExtOldForcing(
                quantity="lateraldischarge",
                filename=DiskOnlyFileModel(None),
                filetype=4,
                method=4,
                operand=Operand.override,
                value=1.0,
        ...
        assert forcing.get_location_id() == expected_result

    @pytest.mark.parametrize(
        "filetype",
        [
            pytest.param("poly", id="polyfile-path"),
            pytest.param("tim", id="timmodel-path"),
            pytest.param("disk", id="diskonlyfilemodel-path"),
            pytest.param("disk-none", id="missing-path"),
        ],
    )
    def test_get_location_filepath(
        self,
        tmp_path: Path,
        tim_files_dir: Path,
        filetype: str,
        ...
        if filetype == "poly":
            expected_result = tmp_path / "myfile.pli"
            expected_result.write_text("named_node\n    1    2\n  0.0  0.0\n")
            forcing = ExtOldForcing(
                quantity="lateraldischarge",
            ...
            expected_result = tim_files_dir / "triple_data_for_timeseries.tim"
            forcing = ExtOldForcing(
                quantity="lateraldischarge",
                filename=expected_result,
                filetype=1,
            ...
            expected_result = tmp_path / "mydisk.xyz"
            expected_result.write_text("")
            forcing = ExtOldForcing(
                quantity="lateraldischarge",
                filename=expected_result,
                filetype=4,
        ...
        assert forcing.get_location_filepath() == expected_result

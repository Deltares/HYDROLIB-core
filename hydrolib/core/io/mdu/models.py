...
class Geometry(INIBasedModel):
    ...
    class Comments(INIBasedModel.Comments):
        ...
        bathymetryfile: Optional[str] = Field("Removed since March 2022. See [geometry] keyword BedLevelFile.", alias="bathymetryFile")
        drypointsfile: Optional[str] = Field("Dry points file <*.xyz>, third column dummy z values, or polygon file <*.pol>.", alias="dryPointsFile")
        structurefile: Optional[str] = Field("File <*.ini> containing list of hydraulic structures. See section C.12.", alias="structureFile")
        inifieldfile: Optional[str] = Field("Initial and parameter field file <*.ini>. See section D.2.", alias="iniFieldFile")
        waterlevinifile: Optional[str] = Field("Initial water levels sample file <*.xyz>.", alias="waterLevIniFile")
        landboundaryfile: Optional[str] = Field("Only for plotting.", alias="landBoundaryFile")
        thindamfile: Optional[str] = Field("<*_thd.pli>, Polyline(s) for tracing thin dams.", alias="thinDamFile")
        fixedweirfile: Optional[str] = Field("<*_fxw.pliz>, Polyline(s) x, y, z, z = fixed weir top levels (formerly fixed weir).", alias="fixedWeirFile")
        pillarfile: Optional[str] = Field("<*_pillar.pliz>, Polyline file containing four colums with x, y, diameter and Cd coefficient for bridge pillars.", alias="pillarFile")
        usecaching: Optional[str] = Field("Use caching for geometrical/network-related items (0: no, 1: yes) (section C.19).", alias="useCaching")
        vertplizfile: Optional[str] = Field("<*_vlay.pliz>), = pliz with x, y, Z, first Z = nr of layers, second Z = laytyp.", alias="vertPlizFile")
        frictfile: Optional[str] = Field("Location of the files with roughness data for 1D. See section C.15.", alias="frictFile")
        crossdeffile: Optional[str] = Field("Cross section definitions for all cross section shapes. See section C.16.", alias="crossDefFile")
        crosslocfile: Optional[str] = Field("Location definitions of the cross sections on a 1D network. See section C.16.", alias="crossLocFile")
        storagenodefile: Optional[str] = Field("File containing the specification of storage nodes and/or manholes to add extra storage to 1D models. See section C.17.", alias="storageNodeFile")
        oned2dlinkfile: Optional[str] = Field("File containing the custom parameterization of 1D-2D links. See section C.18.", alias="1d2dLinkFile")
        proflocfile: Optional[str] = Field("<*_proflocation.xyz>) x, y, z, z = profile refnumber.", alias="profLocFile")
        profdeffile: Optional[str] = Field("<*_profdefinition.def>) definition for all profile nrs.", alias="profDefFile")
        profdefxyzfile: Optional[str] = Field("<*_profdefinition.def>) definition for all profile nrs.", alias="profDefXyzFile")
        manholefile: Optional[str] = Field("File containing manholes (e.g. <*.dat>).", alias="manholeFile")
        partitionfile: Optional[str] = Field("<*_part.pol>, polyline(s) x, y.", alias="partitionFile")
        ...
        bedlevuni: Optional[str] = Field("Uniform bed level [m], (only if bedlevtype>=3), used at missing z values in netfile.", alias="bedLevUni")
        bedslope: Optional[str] = Field("Bed slope inclination, sets zk = bedlevuni + x*bedslope ans sets zbndz = xbndz*bedslope.", alias="bedSlope")
        bedlevtype: Optional[str] = Field("1: at cell center (tiles xz,yz,bl,bob=max(bl)), 2: at face (tiles xu,yu,blu,bob=blu), 3: at face (using mean node values), 4: at face (using min node values), 5: at face (using max node values), 6: with bl based on node values.", alias="bedLevType")
        blmeanbelow: Optional[str] = Field("if not -999d0, below this level [m] the cell centre bedlevel is the mean of surrouding netnodes.", alias="blMeanBelow")
        blminabove: Optional[str] = Field("if not -999d0, above this level [m] the cell centre bedlevel is the min of surrouding netnodes.", alias="blMinAbove")
        anglat: Optional[str] = Field("Angle of latitude S-N [deg], 0=no Coriolis.", alias="angLat")
        anglon: Optional[str] = Field("Angle of longitude E-W [deg], 0=Greenwich Mean Time.", alias="angLon")
        conveyance2d: Optional[str] = Field("-1:R=HU, 0:R=H, 1:R=A/P, 2:K=analytic-1D conv, 3:K=analytic-2D conv.", alias="conveyance2D")
        nonlin1d: Optional[str] = Field("Non-linear 1D volumes, applicable for models with closed cross sections. 1=treat closed sections as partially open by using a Preissmann slot, 2=Nested Newton approach, 3=Partial Nested Newton approach. (For a description of these methods, see (D-Flow FM TRM, 2015, Section 6.9).", alias="nonlin1D")
        nonlin2d: Optional[str] = Field("Non-linear 2D volumes, only i.c.m. ibedlevtype = 3 and Conveyance2D>=1.", alias="nonlin2D")
        sillheightmin: Optional[str] = Field("Fixed weir only active if both ground heights are larger than this value [m].", alias="sillHeightMin")
        makeorthocenters: Optional[str] = Field("(1: yes, 0: no) switch from circumcentres to orthocentres in geominit.", alias="makeOrthoCenters")
        dcenterinside: Optional[str] = Field("limit cell center; 1.0:in cell <-> 0.0:on c/g.", alias="dCenterInside")
        bamin: Optional[str] = Field("Minimum grid cell area [m2], i.c.m. cutcells.", alias="baMin")
        openboundarytolerance: Optional[str] = Field("Search tolerance factor between boundary polyline and grid cells. [Unit: in cell size units (i.e., not meters)].", alias="openBoundaryTolerance")
        renumberflownodes: Optional[str] = Field("Renumber the flow nodes (1: yes, 0: no).", alias="renumberFlowNodes")
        kmx: Optional[str] = Field("Number of vertical layers.", alias="kmx")
        layertype: Optional[str] = Field("Number of vertical layers.", alias="layerType")
        numtopsig: Optional[str] = Field("Number of sigma-layers on top of z-layers.", alias="numTopSig")
        sigmagrowthfactor: Optional[str] = Field("layer thickness growth factor from bed up.", alias="sigmaGrowthFactor")
        dxdoubleat1dendnodes: Optional[str] = Field("Whether a 1D grid cell at the end of a network has to be extended with 0.5Δx.", alias="dxDoubleAt1DEndNodes")
        changevelocityatstructures: Optional[str] = Field("Ignore structure dimensions for the velocity at hydraulic structures, when calculating the surrounding cell centered flow velocities.", alias="changeVelocityAtStructures")
        changestructuredimensions: Optional[str] = Field("Change the structure dimensions in case these are inconsistent with the channel dimensions.", alias="changeStructureDimensions")

...
from hydrolib.core.dflowfm.mdu.models import FMModel, Physics, Processes, Time
...
class MDUParser:
    ...
    def get_temperature_salinity_data(self) -> Dict[str, Any]:
        ...
        # Read active WAQ substance names from the SubstanceFile (if configured).
    ...
    def delete_line(
        ...
        *,
        ...
        Raises:
            ValueError: If both or neither of 'index' and 'keyword' are provided, or if keyword is empty string.
            IndexError: If 'index' is out of bounds for the file content.
        ...
        Examples:
            - Delete a line by index:
                ...
                >>> mdu_file.content[:2]  # doctest: +SKIP
                ['# Generated by the Matlab program dflowfmConverter on 14:39:00, 27-07-2017\n',
                 '# Deltares, Matlab-generated mdu-file for D-Flow FM\n']
                >>> mdu_file.delete_line(index=0)  # doctest: +SKIP
                >>> mdu_file.content[:2]  # doctest: +SKIP
                ['# Deltares, Matlab-generated mdu-file for D-Flow FM\n', '\n']

                ```

            - Delete a line by keyword (case-insensitive):
                ```python
                >>> path = "tests/data/input/mdu-files/extforce_test_1.mdu"
                >>> mdu_file = MDUParser(path)
                >>> mdu_file.content[8:11]
                ['[geometry]\n',
                 '    NetFile                             = sp_net.nc                          # *_net.nc\n',
                 '    unknown_geometry                    = 5                                   # any comment\n']
                >>> mdu_file.delete_line(keyword='NetFile')
                >>> mdu_file.content[8:11]
                ['[geometry]\n',
                 '    unknown_geometry                    = 5                                   # any comment\n',
            ...
            - Delete a line by keyword (case-sensitive) - the function will not find the keyword and will not raise
                ...
                ['[geometry]\n',
                 '    NetFile                             = sp_net.nc                          # *_net.nc\n',
                 '    unknown_geometry                    = 5                                   # any comment\n']
                >>> mdu_file.delete_line(keyword='netfile', case_sensitive=True)
                >>> mdu_file.content[8:11]
                ['[geometry]\n',
                 '    NetFile                             = sp_net.nc                          # *_net.nc\n',
                 '    unknown_geometry                    = 5                                   # any comment\n']
            ...
            - Error if both index and keyword are provided:
                ```python
                >>> path = "tests/data/input/mdu-files/extforce_test_1.mdu"
                >>> mdu_file = MDUParser(path)
                >>> mdu_file.delete_line(index=1, keyword='B') #doctest: +SKIP
                Traceback (most recent call last):
                ...
                ValueError: Exactly one of 'index' or 'keyword' must be provided
                ```
        ...
        See Also:
            ...
            MDUParser.find_keyword_lines: For finding line indices by keyword.
        """
